// STUDY
// 1. for and Object. Run the following code, observe and then answer the questions
    const product = {
    ...
    // x receives property form product
    // 1.2. Use the for loop to print/log out the following output
    for (let x in product) {
        console.log(`${x} : ${product[x]}`);
    }

// 2. Learn about destructuring object in the following tutorials then use one line of code to destructure to obtain subject, dueDate and assignTo from this object:
    const task = {
...
// 3. Here is the actual data from a job search site, copy all of the data, assign it to a variable or a constant named jobSearch in your code. Log or print it out to see its structure then answer the following questions:
    const jobSearch = {
        hits: [
            {
                benefits: [
    ...
    for(let i = 0; i < jobSearch.hits.length; i++){
        console.log(`job's title ${i+1}: ${jobSearch.hits[i]['jobTitle']}`);
        console.log(`locations ${i+1}: ${jobSearch.hits[i]['locations']}`);
        console.log(`salary ${i+1}: ${jobSearch.hits[i]['jobSalary']}`);
        for(let j = 0; j < jobSearch.hits[i]['benefits'].length; j++){
            console.log(`benefits ${i+1}.${j+1}: ${jobSearch.hits[i]['benefits'][j].benefitName}`);
        }
        for(let j = 0; j < jobSearch.hits[i]['benefits'].length; j++){
            console.log(`skills ${i+1}.${j+1}: ${jobSearch.hits[i]['skills'][j]}`);
        }
        console.log(`requirements ${i+1}: \n ${jobSearch.hits[i]['jobRequirement']}`);
    }
...
// 4. Initialize an object to represent a dictionary, with properties as keyword and values as explanation, the initial values are from this table
    ...
    if(dictionary.hasOwnProperty(keyword)){
        alert(`${keyword}\n${dictionary[keyword]}`);
    } else{
    ...
    while(true){
        ...
        if(dictionary.hasOwnProperty(keyword)){
            ...
            let explaination = prompt (`We could not find your word: ${keyword}, leave your explanation:`);
...
// 5. Initialize a variable named products, containing an array of products, each product has a name, price, brand, category, and color
    ...
    for(let i = 0 ; i < products.length ; i++){
        console.log('-----------------------------------');
        console.log(`${Object.keys(products[i])[0]} : ${products[i].Name}`);
        console.log(`${Object.keys(products[i])[2]} : ${products[i].Price}`);        
    }
    ...
    for(let i = 0 ; i < Object.keys(products[position-1]).length ; i++){
        let product = Object.keys(products[position-1])[i];
    ...
    const categoryArray = [];
    for (let i = 0 ; i < products.length ; i++ ){
        let product = products[i][Object.keys(products[i])[4]];
        categoryArray.push(product);
    }
    let userCategory = prompt('Enter your category:');   
    for (let x of products){
        if(x.Category === userCategory){ 
            console.log('-----------------------------------');  
            console.log(`${Object.keys(products[0])[0]} : ${x.Name}`);
            console.log(`${Object.keys(products[0])[2]} : ${x.Price}`)
        }    
    }
